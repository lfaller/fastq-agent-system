...
def benchmark(
    fastq_file: str = typer.Option(
        ...
        help="Path to FASTQ file to benchmark"
    ),
    skip_ai: bool = typer.Option(False, "--skip-ai", help="Skip AI analysis for speed test")
):
    ...
    async def run_benchmark():
        ...
        console.print(f"[green]✅ Parsing completed in {parse_time:.2f} seconds[/green]")
        ...
        if report_result["status"] == "success":
            console.print(f"[green]✅ Report completed in {report_time:.2f} seconds[/green]")
            ...
            console.print(f"  [blue]Processing rate:[/blue] {reads/total_time:.1f} reads/second")
            
            # Show breakdown
            ai_time = report_time - 1.0  # Rough estimate of non-AI time
            if not skip_ai and ai_time > 0:
                console.print(f"  [yellow]Estimated AI time:[/yellow] {ai_time:.2f}s ({ai_time/total_time*100:.1f}% of total)")
...
def generate_report(
    ...
    fast_mode: bool = typer.Option(False, "--fast", help="Skip AI analysis for faster processing"),
    open_report: bool = typer.Option(False, "--open", help="Open directly in a web browser")
):
    ...
    async def run_analysis():
        try:
            ...
            try:
                ...
                console.print(f"[red]Invalid format '{format}'. Use: html, json, markdown[/red]")
                return
            ...
            if parse_result["parsing_status"] != "success":
                console.print(f"[red]❌ Parsing failed: {parse_result.get('error_message')}[/red]")
                return
            ...
            if fast_mode:
                console.print("[blue]⚡ Fast mode enabled - skipping detailed AI analysis[/blue]")
            ...
            if report_result["status"] != "success":
                console.print(f"[red]❌ Report generation failed: {report_result.get('error_message')}[/red]")
                return
...
def test_parser(
    fastq_file: str = typer.Option(
        "tests/fixtures/sample.fastq", 
        help="Path to FASTQ file to test parsing"
    )
):
    ...
    async def run_test():
        ...
        if result["parsing_status"] == "success":
            ...
            console.print(f"[bold red]❌ Parsing failed:[/bold red] {result.get('error_message')}")
