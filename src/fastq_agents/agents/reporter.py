...
class FASTQReportAgent(BaseAgent):
    ...
    async def _create_analysis_report(self, fastq_data: FASTQData, fast_mode: bool = False) -> AnalysisReport:
    ...
    def _generate_fast_analysis(self, report: AnalysisReport) -> Dict[str, Any]:
        ...
        summary = (f"This FASTQ dataset contains {metrics.total_reads:,} reads with "
        ...
        if metrics.average_quality >= 35:
            insights.append("Excellent base call quality indicates reliable sequencing data")
        ...
        if 40 <= metrics.gc_content <= 60:
            insights.append("GC content within normal range suggests no major bias")
        else:
            insights.append(f"GC content ({metrics.gc_content:.1f}%) outside typical range may indicate bias or contamination")
        
        length_var = metrics.max_read_length - metrics.min_read_length
        if length_var <= 5:
            insights.append("Consistent read lengths indicate uniform library preparation")
        elif length_var <= 20:
        ...
        if report.recommendations:
            ...
            if high_priority > 0:
                insights.append(f"{high_priority} high-priority preprocessing steps recommended")
            else:
                insights.append("Standard preprocessing steps recommended for optimal results")
    ...
    async def _generate_ai_analysis(self, fastq_data: FASTQData, report: AnalysisReport) -> Dict[str, Any]:
        ...
        if report.metrics.total_reads < 10:
            return {
                ...
                "insights": [
                    ...
                    f"GC content is {report.metrics.gc_content:.1f}%, which is {'normal' if 40 <= report.metrics.gc_content <= 60 else 'outside normal range'}",
                    f"Read lengths are consistent at {report.metrics.average_read_length:.1f} bp" if report.metrics.max_read_length - report.metrics.min_read_length <= 10 else f"Read lengths vary from {report.metrics.min_read_length} to {report.metrics.max_read_length} bp"
                ],
...
JSON only, no markdown:"""
        ...
        try:
            ...
            try:
                ...
                if all(key in parsed_json for key in ["summary", "insights", "suitability"]):
                    return parsed_json
                else:
                    ...
                    return {
                        ...
                        "suitability": parsed_json.get("suitability", "Review recommended")
                    }
    ...
    async def _generate_recommendations(self, fastq_data: FASTQData, report: AnalysisReport) -> List:
        ...
        if flags.low_quality_reads:
            recommendations.append({
                ...
                "parameters": {"min_quality": 20, "min_length": int(metrics.average_read_length * 0.8)}
            })
        ...
        if flags.uneven_read_lengths:
            recommendations.append({
                ...
                "parameters": {"min_length": metrics.min_read_length + 10, "max_length": metrics.max_read_length - 10}
            })
        
        # GC content recommendations
        if flags.poor_gc_content:
            priority = "high" if metrics.gc_content < 30 or metrics.gc_content > 70 else "medium"
            recommendations.append({
        ...
        if metrics.average_read_length > 50:  # Likely Illumina data
            recommendations.append({
                ...
                "parameters": {"tool": "trimmomatic", "adapter_file": "TruSeq3-PE.fa"}
            })
    ...
    async def _create_report_sections(self, report: AnalysisReport) -> List[ReportSection]:
    ...
    def _create_quality_section_content(self, report: AnalysisReport) -> str:
        ...
        if issues:
            issues_html = "<ul>" + "".join(f"<li>{issue}</li>" for issue in issues) + "</ul>"
        else:
    ...
    async def _save_report(self, report: AnalysisReport, output_dir: Path, format: ReportFormat) -> Path:
    ...
    async def _save_html_report(self, report: AnalysisReport, output_path: Path) -> None:
        ...
        try:
            ...
            quality_value = report.quality_assessment.value if hasattr(report.quality_assessment, 'value') else str(report.quality_assessment)
    ...
    async def _save_simple_html_report(self, report: AnalysisReport, output_path: Path) -> None:
    ...
    async def _save_json_report(self, report: AnalysisReport, output_path: Path) -> None:
        """Save report as JSON."""
    ...
    async def _save_markdown_report(self, report: AnalysisReport, output_path: Path) -> None:
